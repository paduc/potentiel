...
export default function makeAddGarantiesFinancieres({
  ...
  return async function addGarantiesFinancieres({
    ...
    if (!project) {
      logger.error(new Error('addGarantiesFinancières failed because projectRes.is_none()'))
    ...
    if (!updatedProject) {
      logger.error(new Error('addGarantiesFinancieres use-case: applyProjectUpdate returned null'))
