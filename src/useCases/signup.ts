...
export default function makeSignup({
  ...
  return async function signup({
    ...
    if (projectAdmissionKeyResult.is_none()) {
      logger.error(
        new Error(`signup use-case: projectAdmissionKey was not found : ${projectAdmissionKey}`)
      )
    ...
    if (projectAdmissionKeyUpdateRes.is_err()) {
      logger.error(new Error('signup use-case: impossible de mettre à jour projectAdmissionKey'))
    }
