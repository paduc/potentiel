...
const addUserToDrealForTests = async (request: HttpRequest) => {
  ...
  if (!email || !region) {
    logger.error(new Error('addUserToDrealForTests missing email or region'))
  ...
  if (!user) {
    logger.error(new Error('addUserToDrealForTests cant find dreal user with this email'))
